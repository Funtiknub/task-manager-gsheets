...
class GoogleSheetsSync:
    ...
    def получить_колонку_для_даты(self, начальная_дата, целевая_дата, настройки):
        ...
        смещение_колонок = 0
        текущая_неделя = 0
        последняя_пятница = None

        # Проходим по всем дням от начальной до целевой даты
        while текущая_дата <= целевая_дата:
            дата_str = текущая_дата.strftime('%d.%m.%Y')

            # Если это пятница, запоминаем её
            if текущая_дата.weekday() == 4:
                последняя_пятница = текущая_дата
                текущая_неделя += 1
                # Начинаем новую неделю с новой базовой колонки
                if текущая_дата < целевая_дата:
                    смещение_колонок = текущая_неделя * 15  # 15 колонок на неделю

            # Если это рабочий день
            if self.является_рабочим_днем(текущая_дата, настройки):
                # Если это не начало новой недели, добавляем смещение
                if текущая_дата != последняя_пятница:
                    if текущая_дата < целевая_дата:
                        смещение_колонок += 3

            текущая_дата += timedelta(days=1)

        # Получаем итоговую колонку
        индекс_колонки = смещение_колонок
        if индекс_колонки >= len(self.все_колонки):
            raise ValueError(f"Ошибка: требуемая колонка {индекс_колонки} находится за пределами доступного диапазона")
            
        return self.все_колонки[индекс_колонки]

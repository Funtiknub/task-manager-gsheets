...
class СписокДел:
    ...
    def отметить_выполнение_задачи(self):
        """Отмечает задачу как выполненную и добавляет комментарий"""
        try:
            with open('tasks.json', 'r', encoding='utf-8') as file:
                tasks = json.load(file)

            if self.пользователь not in tasks or not tasks[self.пользователь]:
                print("\nНет доступных задач")
                return

            self.показать_задачи(self.пользователь)
            номер = input("\nВведите номер задачи для отметки выполнения: ")
            
            try:
                номер = int(номер) - 1
                if 0 <= номер < len(tasks[self.пользователь]):
                    if not tasks[self.пользователь][номер]['выполнено']:
                        tasks[self.пользователь][номер]['выполнено'] = True
                        новый_комментарий = input("Введите комментарий к выполненной задаче: ")
                        
                        # Получаем существующий комментарий
                        существующий_комментарий = tasks[self.пользователь][номер]['комментарий']
                        
                        # Если уже есть комментарий, добавляем новый через пробел
                        if существующий_комментарий:
                            tasks[self.пользователь][номер]['комментарий'] = f"{существующий_комментарий} {новый_комментарий}"
                        else:
                            tasks[self.пользователь][номер]['комментарий'] = новый_комментарий
    ...
    while True:
        ...
        if выбор == "1":
            while True:
                ...
                if действие == "1":
                    список_дел.добавить_задачи(имя)  # Передаем имя пользователя
                elif действие == "2":
            ...
            список_дел.отметить_выполнение_задачи()
